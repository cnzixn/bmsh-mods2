...
function Dryer:StartDrying(dryable)
	if self:CanDry(dryable) then
	    ...
	    if self.onstartcooking then
			if SaveGameIndex:GetCurrentMode() == "porkland" or SaveGameIndex:GetCurrentMode() == "shipwrecked" or GetPlayer().components.poisonable then
		...
		if self.product then
			if harvester and harvester.components.inventory and not harvester:HasTag("yamcheb") and not harvester:HasTag("critter") then
				...
				if loot then
                    ...
                    elseif SaveGameIndex:GetCurrentMode() == "porkland" or SaveGameIndex:IsModeShipwrecked() or GetPlayer().components.poisonable then
